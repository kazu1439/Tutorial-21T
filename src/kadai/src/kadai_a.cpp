...
説明で出てくる用語の補足
・インスタンス名とは、数学の問題で回答者がとりあえず分からないものを変数xで置くように、
　プログラム作成者が勝手に決めていい変数
　ただ、見返してある程度分かるように変数もxやy,zではなく意味のある変数の名前を付けた方がいいと思う
**********************************/
/**********************************************************************
Include Libraries
**********************************************************************/
...
//制御周期の宣言、2021年信州大学ロボ研は0.02秒で統一、制御周期とはコンピュータ上で読み込まれる時間
...
int main(int argc, char **argv)
{
    ...
    // ちょっと前に決めた制御周期を参照し制御周期の実行を行っている

    msg_int.data.resize(3);
    //36行目で宣言した配列の要素数を決めている
    ...
    while (ros::ok())
    {
        ...
        loop_rate.sleep();
        //定めた制御周期にするためにここでプログラムを一時的に停止させている
